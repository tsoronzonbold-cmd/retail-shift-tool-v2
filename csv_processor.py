...
def auto_detect_columns(df_columns):
    ...
    """
    mapping = {}
    # Combined patterns from our tool + Christian's aliases + every CSV variant
    # we've seen from partners. Goal: never miss a column again.
    patterns = {
        "retailer": [r"^retailer$", r"^retailer\s*name$", r"^chain$", r"^banner$",
                     r"^store\s*name$", r"^brand$", r"^company\s*name$"],
        "store_number": [r"store\s*#", r"store\s*num", r"store\s*no", r"location\s*#",
                         r"^store$", r"^business$", r"business\s*name", r"^#$", r"^loc"],
        "address": [r"^address", r"street", r"addr"],
        "city": [r"^city", r"^metro$"],
        "state": [r"^state"],
        "zip": [r"^zip", r"postal", r"^zip\s*code$"],
        "start_date": [r"start\s*date", r"^date$", r"visit\s*date", r"^visitdt$",
        ...
        "quantity": [r"^quantity", r"#\s*of\s*worker", r"\bqty\b", r"headcount",
                     ...
                     r"#\s*of\s*pros", r"^pros$", r"^head.?ct$"],
        "requested_workers": [r"requested\s*workers?", r"workers?\s*requested",
                              r"any\s*requested", r"name\s*of\s*pros",
                              r"requested\s*pros?", r"any\s*requested\s*pros",
                              r"^pro.?names$"],
        "position": [r"^position$", r"^role$", r"^job\s*title$"],
        "schedule_name": [r"schedule\s*name", r"^schedule$", r"shift\s*name"],
        "team_lead": [r"on.?site\s*contact$", r"team\s*lead(?!\s*phone|\s*email)",
                      r"^contact$", r"^supervisor$", r"^lead$", r"^supervisors$",
        ...
        "worker_pay_rate": [r"worker\s*pay\s*rate", r"pay\s*rate", r"hourly\s*rate",
                            ...
                            r"pro\s*rate", r"requested\s*rate"],
